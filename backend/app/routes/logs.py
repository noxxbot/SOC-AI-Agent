...
async def ingest_logs(payload: LogIngestRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    skipped_invalid = 0
    event_ids = list({str(log.get("event_id")) for log in payload.logs if log.get("event_id")})
    ...
    for log in payload.logs:
        ...
        timestamp = parse_timestamp(log.get("timestamp")) or datetime.now(timezone.utc)

        to_insert.append(
            EndpointLog(
                event_id=event_id,
                timestamp=timestamp,
                agent_id=str(log.get("agent_id") or payload.agent_id),
                hostname=log.get("hostname") or payload.hostname,
    ...
    ingest_ts = payload.timestamp or datetime.now(timezone.utc).isoformat()
    processed_result = process_logs_batch(payload.agent_id, payload.hostname or "unknown", ingest_ts, payload.logs)
    ...
    for log in unique_processed:
        ...
        ts_value = parse_timestamp(log.get("timestamp")) or parse_timestamp(ingest_ts) or datetime.now(timezone.utc)

        processed_to_insert.append(
            ProcessedLog(
                agent_id=str(log.get("agent_id") or payload.agent_id or "unknown"),
                hostname=log.get("hostname") or payload.hostname,
                timestamp=ts_value,
